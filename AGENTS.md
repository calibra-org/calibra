...
Full operator guide: [`scripts/spin.md`](scripts/spin.md). Design rationale: [#21](https://github.com/calibra-org/calibra/issues/21).
...
For worktree-based spins, the same commands accept a slug:

```sh
pnpm spin doctor <slug> --json
pnpm spin list --json
pnpm spin url <slug> grafana
pnpm spin logs <slug> api.ndjson
pnpm spin metrics <slug>
pnpm spin alerts <slug>
```
