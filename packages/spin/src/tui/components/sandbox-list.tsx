...
export function SandboxList({ rows, selected }: { rows: SandboxRow[]; selected: number }) {
    return (
        <Box flexDirection="column" flexGrow={1} borderStyle="round" borderColor={theme.muted} paddingX={1}>
            <Text color={theme.muted}>{cell("SANDBOX", 22)} {cell("STATUS", 12)} ADMIN  API</Text>
            ...
            {rows.map((row, index) => {
                ...
                return (
                    <Text key={row.slug} inverse={active}>
                        {active ? "▸ " : "  "}
                        <Text color={theme.title}>{cell(row.slug, 20)}</Text> <Text color={statusColor(row.status)}>{cell(row.status, 12)}</Text>{" "}
                        {row.admin}  {row.api}
                    </Text>
                );
            })}
