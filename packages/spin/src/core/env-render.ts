...
export function renderCaddyfile(meta: SpinMeta): string {
    ...
    const blocks = servicesForProfile(meta.profile).flatMap((service) => serviceCaddyBlocks(meta, service));
    return `${global}\n${blocks.join("\n")}`;
}
