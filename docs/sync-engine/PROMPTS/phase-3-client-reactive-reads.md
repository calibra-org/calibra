...
Start the phase on the **parallel `sync-engine` track** — do NOT land on `main`:
    ...
    git fetch origin && git merge origin/sync-engine   # fold in 00-foundation + Phases 1–2
...
`pnpm spin doctor sync-engine-phase-3 --json`. Commit + push; merge the phase PR **into
