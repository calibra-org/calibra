...
test.group("Admin product detail extensions", (group) => {
    ...
    test("PATCH syncs upsells / cross-sells / grouped members in order", async ({ client, assert }) => {
        ...
        const response = await client.patch(`/api/v1/admin/products/${subject.id}`).json({
    ...
    test("downloads replace-all upserts new and removes missing", async ({ client, assert }) => {
        ...
        let response = await client.patch(`/api/v1/admin/products/${subject.id}`).json({
            downloads: [
        ...
        response = await client.patch(`/api/v1/admin/products/${subject.id}`).json({
    ...
    test("pos_available round-trips through the validator + transformer", async ({ client, assert }) => {
        ...
        const response = await client.patch(`/api/v1/admin/products/${subject.id}`).json({ pos_available: false });
        response.assertStatus(200);
        response.assertAgainstApiSpec();
    ...
    test("check-slug returns false for taken slug", async ({ client, assert }) => {
        ...
        const response = await client.get("/api/v1/admin/products/check-slug?slug=duplicate-en&locale=en");
        response.assertStatus(200);
        response.assertAgainstApiSpec();
    ...
    test("check-slug returns true for free slug", async ({ client, assert }) => {
        const response = await client.get("/api/v1/admin/products/check-slug?slug=brand-new-slug&locale=en");
        response.assertStatus(200);
        response.assertAgainstApiSpec();
