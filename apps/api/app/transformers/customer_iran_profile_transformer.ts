...
/**
 * Pattern 3 extension serializer. Only used when a `customer_iran_profiles` row actually exists for
 * the customer — callers gate on row presence so the parent transformer can decide whether to add
 * the `profile_extensions.iran` key at all (vs. emitting `null` for non-Iranian customers).
 */
