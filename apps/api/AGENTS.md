...
- **Caddy** terminates TLS for every service with an internal CA. All UIs reachable at `https://<service>.<slug>.calibra.localhost`. Run `caddy trust` once on your machine to trust the local root. Bare ports still work as escape hatches; the hostname route is the prod-parity path.
- **GlitchTip** (Sentry-protocol) — errors thrown via `@sentry/node` show up at `errors.<slug>.calibra.localhost`. The DSN is written to `apps/api/.env` as `GLITCHTIP_DSN` once the operator registers + creates an org/project on first run (auto-provisioning is on the roadmap; the handoff card prints the one-time setup blurb). Anything you'd report in prod, report locally too — that's how we catch silent error-handler regressions.
...
- **Meilisearch** runs on `search.<slug>.calibra.localhost`. The api auto-discovers it via `MEILISEARCH_HOST` + `MEILISEARCH_API_KEY` (`config/env.ts`); the singleton client lives in `app/services/meilisearch.ts`. Reindex flows go through the search-index service (lands in a follow-up PR).
