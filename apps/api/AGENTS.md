...
- **`@adonisjs/otel` + `@sentry/node`** — both env-gated. `OTEL_EXPORTER_OTLP_ENDPOINT` enables tracing; `SENTRY_DSN` enables error reporting. `bin/sentry.ts` initialises Sentry before the Ignitor boots; the global exception handler forwards via `Sentry.captureException` (tagged with the request id) when `shouldReport` agrees.
