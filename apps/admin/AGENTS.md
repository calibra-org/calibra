...
- **Page data goes through `lib/server-repos.ts`.** Pages never call `apiServer()` directly — they import `listProducts`, `getOrder`, etc. from `#/lib/server-repos`. Those functions call the SDK and adapt the response into the camelCase view types in `#/lib/types`. When a screen needs a new endpoint, add a function to `server-repos.ts` rather than putting the SDK call in the page.
