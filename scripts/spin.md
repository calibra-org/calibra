...
| `pnpm spin start <slug> [--with-web] [--no-pr]` | Bootstrap (or resume) a worktree-based spin. `--with-web` also runs the storefront on the allocated web port. `--no-pr` skips the draft-PR step — call `pnpm spin pr <slug>` later. |
| `pnpm spin local [start\|stop\|status]` | **In-place** spin against the current checkout — no worktree, no branch, no PR. Same prod-parity stack. Refuses to start if `apps/api/.env` exists and wasn't generated by spin. `just spin` wraps this. |
...
| Command | What it does |
| --- | --- |
| `pnpm spin list [--json]` | List every persisted spin with status (running / partial / stopped) and ports. JSON for scripting. |
| `pnpm spin doctor <slug> [--json]` | Per-spin status: each service up/down, urls, queue worker, PR. JSON variant returns a flat report agents can `jq` over. |
| `pnpm spin url <slug> [service]` | Print one URL to stdout. Default service: `dashboard`. Recognised: `api`, `admin`, `web`, `db`, `pgadmin`, `mailpit-{web,smtp}`, `redis`, `redisinsight`, `adminer`, `caddy`, `meilisearch`, `grafana`, `prometheus`, `loki`, `tempo`, `alertmanager`, `glitchtip`, `uptime-kuma` (aliases: `mail`, `prom`, `meili`, `errors`, `uptime`). |
| `pnpm spin logs <slug> [stream]` | Print the absolute log path for a stream. Default: `api.ndjson`. Recognised: `api.ndjson`, `api`, `admin`, `web`, `queue`, `agent`. The caller picks `tail` / `less` / `jq`. |
| `pnpm spin metrics <slug>` | curl the api's `/metrics` → stdout. Exit 0 on success, 2 if the api is unreachable. |
| `pnpm spin alerts <slug>` | Query Prometheus `/api/v1/alerts` via the spin's Caddy hostname → stdout (JSON). |

**Teardown**

| Command | What it does |
| --- | --- |
| `pnpm spin stop <slug> [--purge] [--remove] [--force]` | Stop dev servers + containers. `--purge` also drops the docker volumes (wipes the seeded DB). `--remove` deletes the worktree + branch (refuses on uncommitted work; bypass with `--force`). |
| `pnpm spin local stop [--purge]` | Tear down the in-place spin. `just spin-down` wraps this. |

## Codebase layout (`scripts/spin/`)

The CLI is decomposed into single-purpose modules so a function is easy to find without grepping a 2k-line file:

```
scripts/spin.mjs                    # 5-line entrypoint
scripts/spin/
  cli.mjs                           # argv parsing + dispatch + error handler
  paths.mjs · log.mjs · flags.mjs · run.mjs    # leaf helpers
  ports.mjs · probes.mjs · meta.mjs            # state + probe primitives
  env-files.mjs · observability-config.mjs     # template rendering
  compose.mjs · worktree.mjs · install.mjs ·
    migrate.mjs · processes.mjs · pr.mjs ·
    handoff.mjs · help.mjs                     # bootstrap stages
  commands/
    start.mjs · stop.mjs · local.mjs · doctor.mjs · list.mjs ·
    url.mjs · logs.mjs · metrics.mjs · alerts.mjs
```

Zero dependencies — the script bootstraps before `pnpm install` lands.
