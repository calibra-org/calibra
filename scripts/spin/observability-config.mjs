...
export function renderCaddyfile(meta) {
    ...
    /** Control-plane console (Phase 5) — a single global host, no tenant label. Omitted for metas
     * allocated before the `platform` port. */
    const consoleBlock =
        platformPort === null
            ? ""
            : `
console.${slug}.spin.localhost {
    tls internal
    reverse_proxy host.docker.internal:${platformPort}
}
`;
