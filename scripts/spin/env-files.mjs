...
export async function ensureEnvFiles(meta) {
    ...
    await writeFile(
        ...
        [
            ...
            `NEXT_PUBLIC_CONSOLE_URL=http://localhost:${meta.ports.platform ?? meta.ports.web ?? meta.ports.admin}`,
            "",
        ].join("\n"),
    );
    ...
    /**
     * Control-plane console env file (Phase 5). Global app — no tenant root. Only written when the
     * `platform` port is allocated (metas predating the role skip it).
     */
    if (typeof meta.ports.platform === "number") {
        await writeFile(
            join(meta.worktreePath, "apps/platform/.env.local"),
            [
                `# Generated by scripts/spin.mjs for spin "${meta.slug}". Safe to edit; re-running spin overwrites.`,
                `NEXT_PUBLIC_API_BASE_URL=http://localhost:${meta.ports.api}`,
                `NEXT_PUBLIC_SITE_URL=http://localhost:${meta.ports.platform}`,
                `NEXT_PUBLIC_DEFAULT_LOCALE=fa`,
                /** Per-tenant Grafana ops embed (Phase 6 wires the dashboard + `var-tenant`); empty
                 * here so the console renders the placeholder slot until then. */
